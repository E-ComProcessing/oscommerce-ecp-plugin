<?php
/**
 * E-ComProcessing Standard Notifications
 *
...
require 'includes/apps/ecomprocessing/libs/genesis_php/vendor/autoload.php';

use \Genesis\Genesis as Genesis;
use \Genesis\GenesisConfig as GenesisConfig;

if ( !defined('MODULE_PAYMENT_ECOMPROCESSING_STANDARD_STATUS') || (MODULE_PAYMENT_ECOMPROCESSING_STANDARD_STATUS  != 'True') ) {
	exit(0);
}

function setCredentials() {
	GenesisConfig::setUsername(
		getConst('MODULE_PAYMENT_ECOMPROCESSING_STANDARD_USERNAME')
	);
	GenesisConfig::setPassword(
		getConst('MODULE_PAYMENT_ECOMPROCESSING_STANDARD_PASSWORD')
	);
	GenesisConfig::setToken(
		getConst('MODULE_PAYMENT_ECOMPROCESSING_STANDARD_TOKEN')
	);

	switch(getConst('MODULE_PAYMENT_ECOMPROCESSING_STANDARD_ENVIRONMENT')){
		default:
		case 'Staging':
	...
	if ($notification->isAuthentic()) {
		...
		if (isset($reconcile->transaction_id)) {
			...
			switch ($reconcile->status) {
				case 'approved':
					$order_status_id = intval(getConst('MODULE_PAYMENT_ECOMPROCESSING_STANDARD_PROCESSED_ORDER_STATUS_ID'));
					break;
				case 'error':
				case 'declined':
					$order_status_id = intval(getConst('MODULE_PAYMENT_ECOMPROCESSING_STANDARD_FAILED_ORDER_STATUS_ID'));
					break;
				default:
					$order_status_id = intval(getConst('MODULE_PAYMENT_ECOMPROCESSING_STANDARD_ORDER_STATUS_ID'));
			}
