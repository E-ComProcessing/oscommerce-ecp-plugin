...
if ( !defined('MODULE_PAYMENT_ECOMPROCESSING_GENESIS_STATUS') || (MODULE_PAYMENT_ECOMPROCESSING_GENESIS_STATUS  != 'True') ) {
	exit(0);
}
...
if (isset($_POST['unique_id'])) {
	...
	EComProcessing_Genesis_setCredentials();
	...
	if ($notification->isAuthentic()) {
		...
		if (isset($reconcile->transaction_id)) {
			...
			$order_status_id = intval(getConst('MODULE_PAYMENT_ECOMPROCESSING_GENESIS_ORDER_STATUS_ID'));
...
function EComProcessing_Genesis_setCredentials() {
	GenesisConfig::setUsername(
		getConst('MODULE_PAYMENT_ECOMPROCESSING_GENESIS_USERNAME')
	);
	GenesisConfig::setPassword(
		getConst('MODULE_PAYMENT_ECOMPROCESSING_GENESIS_PASSWORD')
	);
	GenesisConfig::setToken(
		getConst('MODULE_PAYMENT_ECOMPROCESSING_GENESIS_TOKEN')
	);

	switch(getConst('MODULE_PAYMENT_ECOMPROCESSING_GENESIS_ENVIRONMENT')){
		default:
		case 'Staging':
