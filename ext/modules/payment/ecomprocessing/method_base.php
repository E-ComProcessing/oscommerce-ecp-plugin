 ...
 * Class ecomprocessing_method_base
 */
abstract class ecomprocessing_method_base extends ecomprocessing_base
{
    /**
     * Return common usage
     * @const string
    ...
    /**
     * ecomprocessing_method_base constructor.
     */
    ...
    protected function init() {
        ...
        if ($this->getIsInstalled()) {
            if (!$this->getIsConfigured()) {
                $this->title .= '<span class="error-ecomprocessing"> (Not Configured)</span>';
            } elseif (!$this->getIsEnabled()) {
                $this->title .= '<span class="error-ecomprocessing"> (Disabled)</span>';
            } elseif ($this->getModuleRequiresSSL() && !$this->getIsSSLEnabled()) {
                $this->title .= '<span class="error-ecomprocessing"> (SSL NOT Enabled)</span>';
                $this->description =
                    '<div class="secError">' .
                ...
                $this->title .= '<span class="warning-ecomprocessing"> (Staging Mode)</span>';
            } else {
                $this->title .= '<span class="success-ecomprocessing"> (Live Mode)</span>';
            }
        }
    ...
    protected function executeReferenceTransaction($transactionType, $data, $initialTransaction)
    {
        ...
        if ($transactionType == $captureType || $transactionType == $refundType) {
            $klarnaData  = ecp_get_klarna_data($initialTransaction['order_id']);
            $klarnaItems = ecp_get_klarna_custom_param_items($klarnaData, true);
    ...
    public function displayTransactionsPanel($order_id)
    {
        ...
        $data = new stdClass;
        $data->paths = array(
            'images' => DIR_FS_ADMIN . 'images/ecomprocessing/',
            'js' => 'includes/javascript/ecomprocessing/js/',
            'css' => 'includes/javacript/ecomprocessing/css/'
        );
    ...
    {
        $files = array(
            'functions_ecomprocessing.php',
            'functions_ecomprocessing_cfg.php'
        );

        foreach ($files as $file) {
            $path = defined('DIR_FS_ADMIN') ? DIR_FS_ADMIN : DIR_FS_CATALOG . '/admin/';
            $file = $path . "ext/modules/payment/ecomprocessing/{$file}";
    ...
    {
        $this->title .= '<span class="error-ecomprocessing"> (Error)</span>';
        $this->description =
            '<div class="secError">' .
            $errorMessage .
            '</div>' .
    ...
    protected function getConfigurationValues()
    {
        return array(
            ...
            array(
                ...
                "6",
                "70",
                "ecp_zfg_draw_number_input({$this->sortOrderAttributes}, ",
                null
            ),
            array(
                "Payment Zone",
                ...
                "6",
                "75",
                "ecp_cfg_pull_down_zone_classes(",
            ...
            array(
                ...
                "6",
                "80",
                "ecp_zfg_pull_down_order_statuses(",
    ...
    protected function doCheckAndPatchOrdersCoreTemplateFile($shouldOnlyCheckIfPatched = false)
    {
        ...
        $orderTransactionsPanelAutoLoadSearchBlock =
            "require_once \$ecpOrderTransactionsPanelFile;";

        $orderTransactionsPanelAutoLoadIncludeBlock =
            "\$ecpOrderTransactionsPanelFile = DIR_FS_ADMIN . \"ext/modules/payment/ecomprocessing/order_transactions_panel.php\";
  if (file_exists(\$ecpOrderTransactionsPanelFile)) {
      {$orderTransactionsPanelAutoLoadSearchBlock}
  }
  ";
    ...
    protected function getNotificationUrl()
    {
        ...
        return
            tep_href_link("ext/modules/payment/ecomprocessing/{$model}.php", "", "SSL");
    }
    ...
    {
        return
            tep_href_link("ext/modules/payment/ecomprocessing/redirect.php", "return={$action}", "SSL");
    }
