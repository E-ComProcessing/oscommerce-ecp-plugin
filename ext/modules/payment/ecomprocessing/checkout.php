<?php
/**
 * E-ComProcessing Checkout Notifications
 *
...
if ( !defined('MODULE_PAYMENT_ECOMPROCESSING_CHECKOUT_STATUS') || (MODULE_PAYMENT_ECOMPROCESSING_CHECKOUT_STATUS  != 'True') ) {
	exit(0);
}

function setCredentials() {
	GenesisConfig::setUsername(
		getConst('MODULE_PAYMENT_ECOMPROCESSING_CHECKOUT_USERNAME')
	);
	GenesisConfig::setPassword(
		getConst('MODULE_PAYMENT_ECOMPROCESSING_CHECKOUT_PASSWORD')
	);

	switch(getConst('MODULE_PAYMENT_ECOMPROCESSING_CHECKOUT_ENVIRONMENT')){
		default:
		case 'Staging':
	...
	if ($notification->isAuthentic()) {
		...
		if (isset($reconcile->payment_transaction)) {
			...
			switch ($payment->status) {
				case 'approved':
					$order_status_id = intval(getConst('MODULE_PAYMENT_ECOMPROCESSING_CHECKOUT_PROCESSED_ORDER_STATUS_ID'));
					break;
				case 'error':
				case 'declined':
					$order_status_id = intval(getConst('MODULE_PAYMENT_ECOMPROCESSING_CHECKOUT_FAILED_ORDER_STATUS_ID'));
					break;
				default:
					$order_status_id = intval(getConst('MODULE_PAYMENT_ECOMPROCESSING_CHECKOUT_ORDER_STATUS_ID'));
			}
