...
if (!class_exists('ecomprocessing_threeds')) {
    require_once DIR_FS_CATALOG . 'ext/modules/payment/ecomprocessing/ecomprocessing_threeds.php';
}
...
class ecomprocessing_checkout extends ecomprocessing_method_base
{
    ...
    protected function getConfigurationValues()
    {
        $configurationValues = array(
            ...
            array(
                ...
                '6',
                '65',
                "ecp_zfg_select_drop_down_single_from_object(\"{$this->code}\", \"getConfigChallengeIndicators\",",
                null
            ),
            array(
                'SCA Exemption',
                ...
                '6',
                '65',
                "ecp_zfg_select_drop_down_single_from_object(\"{$this->code}\", \"getConfigScaExemption\",",
                null
            ),
            array(
                'Exemption Amount',
    ...
    private function setThreedsData($request, $data)
    {
        ...
        $customer_info                    = ecomprocessing_threeds::getCustomerInfo($customer_id);
        $customer_orders                  = ecomprocessing_threeds::getCustomerOrders($customer_id);
        $orders_for_a_period              = ecomprocessing_threeds::findNumberOfOrdersForaPeriod($customer_orders);
        ...
        $request
            ...
            ->setThreedsV2PurchaseCategory(ecomprocessing_threeds::getThreedsPurchaseCategory($isVirtualCart))
            ->setThreedsV2MerchantRiskDeliveryTimeframe(
                ecomprocessing_threeds::getThreedsDeliveryTimeframe($isVirtualCart)
            )
            ->setThreedsV2MerchantRiskShippingIndicator(ecomprocessing_threeds::getShippingIndicator($data))
            ->setThreedsV2MerchantRiskReorderItemsIndicator(
                ecomprocessing_threeds::getReorderItemsIndicator($customer_id, $data->order->products)
            )
            ...
            ->setThreedsV2CardHolderAccountPasswordChangeIndicator(
                ecomprocessing_threeds::getPasswordChangeIndicator($customer_info['date_account_last_modified'])
            )
            ->setThreedsV2CardHolderAccountLastChangeDate($customer_info['date_account_last_modified'])
            ->setThreedsV2CardHolderAccountUpdateIndicator(ecomprocessing_threeds::getUpdateIndicator($customer_info))
            ->setThreedsV2CardHolderAccountRegistrationDate(
                ecomprocessing_threeds::findFirstCustomerOrderDate($customer_orders)
            )
            ->setThreedsV2CardHolderAccountRegistrationIndicator(
                ecomprocessing_threeds::getRegistrationIndicator($customer_orders)
            )
        ...
        if (!$isVirtualCart) {
            $shipping_address_date_first_used = ecomprocessing_threeds::findShippingAddressDateFirstUsed(
                $data->order->delivery,
                $customer_orders
            );
            ...
            $request
                ...
                ->setThreedsV2CardHolderAccountShippingAddressUsageIndicator(
                    ecomprocessing_threeds::getShippingAddressUsageIndicator($shipping_address_date_first_used)
                );
        }
    }
